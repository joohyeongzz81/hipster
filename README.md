...
- 차트 서빙을 `chart_scores` 기반 조회 계층, Redis, Elasticsearch, MySQL fallback, 버전 기준 메타데이터로 나눠 조회 부하가 큰 경로를 안정화
- `AFTER_COMMIT + RabbitMQ` 기반으로 쓰기 경로와 집계 갱신을 분리하고, 전체 재집계 배치로 파생 집계를 복구할 수 있는 구조를 설계
- 차트 재생성과 공개를 `stage` 기반 스냅샷 생성 단계와 버전 기반 공개 단계로 분리
...
- 차트 결과 2,000건 청크 평균 처리 시간: **836.67ms → 434.33ms**
