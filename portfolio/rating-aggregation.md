...
> 이 테이블은 원본 `ratings`와 downstream 차트 계층 사이에서, 평점 조회 성능, 짧은 쓰기 경로, 유저 가중치 변경 흡수, 전체 재집계 기반 복구 가능성까지 함께 맡는 공통 집계 계층입니다.
...
- 원본 `ratings` 직접 집계는 이 공개 지표의 조회 비용을 키웠고, 이를 평점 행 단위 비정규화로 풀면 쓰기 증폭이 열렸습니다.
...
첫째, `ratings`를 끝까지 원본 기준 데이터로 유지해야 했습니다.  
...
- `ratings`
  - 원본 평점과 변경 이력을 보존하는 원본 기준 데이터입니다.
...
- downstream 차트 계층
  - `release_rating_summary`를 upstream으로 읽어 차트용 projection을 계산합니다.

이 구조의 핵심은, 조회 성능과 파생 집계 책임을 `release_rating_summary`라는 하나의 공통 집계 계층으로 모았다는 점입니다.  
그래야 유저 가중치 변화도 과거 평점 행 재기록이 아니라, 이후 집계 경로가 최신 `users.weighting_score`를 읽어 summary를 다시 계산하는 방식으로 흡수할 수 있습니다.
...
### 3-1. `release_rating_summary`로 조회 핫패스를 분리했지만, 동기 쓰기 병목이 드러났습니다
...
- 원본 평점 저장과 파생 집계 갱신이 같은 트랜잭션에 묶였습니다.
  - summary 행에 대한 경쟁이 API 응답 시간으로 직접 전파됐습니다.
  - 읽기 문제를 푼 대신 쓰기 경로가 길어지는 병목이 생겼습니다.

실측에서도 이 중간 병목은 분명하게 드러났습니다.  
동일 데이터셋에서 단일 평점 등록 API를 기준으로 보면, summary를 원본 쓰기 경로와 동기 결합했을 때 개별 평점 등록 API 응답 시간은 `7ms -> 126ms`로 악화됐습니다.

여기서 결론도 명확하게 드러났습니다.  
`release_rating_summary`는 필요했지만, 이 계층을 원본 쓰기와 동기 결합한 채로는 운영하기 어려웠습니다.

### 3-2. 그래서 쓰기 경로와 집계 갱신을 `AFTER_COMMIT + RabbitMQ`로 분리했습니다
...
이렇게 바꾸면 원본 쓰기 경로는 `ratings` 반영까지만 책임지고, `release_rating_summary` 갱신은 커밋 이후 비동기 작업으로 밀려납니다.  
RabbitMQ는 여기서 주인공이 아니라, 공통 집계 계층을 짧은 쓰기 경로와 함께 운영하기 위한 분리 장치입니다.

이번 선택에서 먼저 세운 기준은 네 가지였습니다.

- 원본 쓰기 경로를 짧게 유지할 것
- 프로세스 장애와 분리된 비동기 경계를 둘 것
- summary 갱신 실패를 원본 쓰기 경로와 격리할 것
...
Kafka는 replay, 장기 보관, 다중 consumer 확장에는 강하지만, 현재처럼 `release_rating_summary` 갱신 작업을 원본 쓰기 경로에서 분리하는 문제에는 요구사항 대비 운영 복잡도가 더 컸습니다.  
그래서 현재 규모에서는 RabbitMQ의 작업 큐 모델을 통해 쓰기 경로를 짧게 유지하면서도, 프로세스와 분리된 전달 경계, 소비 제어, 실패 격리를 가져가는 쪽이 가장 균형이 좋다고 판단했습니다.

운영 측면에서는 아래 장치도 함께 들어갔습니다.

- summary 소비자는 manual ack로 성공/실패를 명시적으로 처리합니다.
- 영구 실패는 DLQ로 분리해 쓰기 경로 전체를 오염시키지 않게 했습니다.
...
`AntiEntropyBatchJob`은 release id를 청크로 순회하면서 `ratings JOIN users`를 다시 읽고, `total_rating_count`, `average_score`, `weighted_score_sum`, `weighted_count_sum`, `batch_synced_at`를 한 번에 덮어씁니다. 마지막 평점이 삭제된 릴리즈까지 정리할 수 있도록 오래된 summary 행 삭제도 함께 처리합니다.
...
### 3-4. 유저 가중치 변경도 과거 평점 행 재기록 없이 이 계층이 흡수하게 만들었습니다

이 계층이 중요한 이유는 조회 성능뿐 아니라, 유저 가중치 변경을 과거 평점 행 재기록 없이 흡수하게 만들기 때문입니다.

현재 유저 가중치 배치는 `users.weighting_score`와 `user_weight_stats`만 갱신합니다.  
그 다음 반영은 과거 `ratings` 행을 다시 쓰는 방식이 아니라, 이후 평점 집계 경로가 최신 `users.weighting_score`를 읽어 `release_rating_summary`를 다시 계산하는 방식으로 처리합니다.

즉, 전파 경로는 `가중치 변경 -> ratings 행 재기록`에서 `가중치 변경 -> summary 재계산 -> downstream 차트 반영`으로 바뀌었습니다.  
이 덕분에 차트 배치도 원본 `ratings`나 평점 행 단위 비정규화 값 대신, 더 안정적인 upstream 집계 계층을 읽게 됐습니다.
...
- 원본 직접 조회와 평점 행 단위 비정규화 사이에서 흔들리던 책임을 `release_rating_summary`로 재배치했습니다.
- 조회 경로와 쓰기 경로를 분리해 조회 최적화가 쓰기 병목으로 되돌아오지 않게 만들었습니다.
...
ratings
  ...
  -> RabbitMQ
  -> release_rating_summary 증분 갱신

ratings + users.weighting_score
  -> Anti-Entropy 전체 재집계
  -> release_rating_summary 덮어쓰기 / 보정

release_rating_summary
  -> release detail/list/search 조회 경로
  -> downstream 차트 projection 입력
```
...
- 평점 등록 쓰기 경로를 짧게 유지하면서도 summary 갱신을 분리할 수 있게 했습니다.
- 유저 가중치 변경을 과거 평점 행 재기록이 아니라 summary 재계산으로 흡수하게 만들었습니다.
...
이번 단계에서는 Kafka급 이벤트 로그 플랫폼이나 Outbox까지 바로 열기보다, 현재 규모에서 짧은 쓰기 경로와 원본 기준 복구 가능성을 우선했습니다.  
...
이 문서의 범위는 `release_rating_summary`를 중심으로 한 공통 집계 계층의 책임, 반영 경계, 복구 경로까지입니다.  
그 다음 단계의 차트 publish 경계와 공개 일관성은 관련 문서에서 이어서 설명합니다.
...
아래 문서는 이 공통 집계 계층과 맞닿은 관련 문서입니다.
...
`release_rating_summary`를 단순 조회 캐시가 아니라, 평균 평점과 차트 upstream을 함께 맡는 공통 집계 계층으로 두면서 원본 데이터, 쓰기 경로, downstream 집계, 복구 경로의 책임을 다시 정리했습니다.
