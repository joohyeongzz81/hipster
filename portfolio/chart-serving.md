...
갱신 시각은 검색 결과와 무관한 공통 값인데도 당시에는 요청마다 같은 조회 비용이 함께 붙어 있었습니다. 그래서 검색 후보를 아무리 빨리 찾아도, 공통 메타데이터 확인이 끝날 때까지 실제 응답은 계속 묶여 있었습니다.
...
위 흐름을 채용 담당자 관점에서 다시 압축하면, 대표 시나리오는 아래 여섯 줄로 기억할 수 있습니다. 각 행은 같은 시나리오에서 가장 의미 있는 before/after를 한 줄로 묶은 값입니다.
