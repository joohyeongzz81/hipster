...
차트 API는 단순 상위권 조회가 아니라, `bayesian_score DESC` 기준으로 필터 결과를 정렬해 보여주고 `lastUpdated`까지 함께 반환하는 검색성 조회 경로였습니다. 병목도 `chart_scores` 조회 하나에만 있지 않았습니다. 릴리즈·분류 정보 보강, 캐시 미스, 검색, 갱신 시각 조회가 한 응답 경로에 함께 얽혀 있었습니다.

같은 API와 같은 데이터셋에서 단계별 병목 이동을 확인하며 비정규화, MySQL 최적화, Redis 응답 캐시, Elasticsearch 검색, 메타데이터 분리를 순서대로 적용했습니다. 그 결과 차트 API를 하나의 느린 조회가 아니라 응답 경로 전체의 문제로 다시 정의했고, 최종적으로 조회 경로를 캐시·검색·폴백 경로·메타데이터 경로로 나눠 응답 병목을 줄였습니다.
