# 차트 API 조회 경로를 캐시·검색·폴백·메타데이터로 분리해 응답 병목 줄이기

차트 API는 단순 상위권 조회가 아니라, `bayesian_score DESC` 기준으로 필터 결과를 정렬해 보여주고 `lastUpdated`까지 함께 반환하는 검색성 조회 경로였습니다. 이 문서는 `chart_scores` 기반 조회 경로를 단계별 벤치마크로 분해해, 병목이 실제로 어디서 어디로 이동하는지 확인한 뒤 서빙 경로를 다시 나눈 과정을 정리합니다.

- **상황**: `chart_scores` 조회, 릴리즈·분류 정보 보강, 캐시 미스, 갱신 시각 조회가 한 경로에 몰려 있어 응답 병목이 겹쳐 있었습니다.
- **행동**: 비정규화, MySQL 최적화, Redis 응답 캐시, Elasticsearch 검색, 메타데이터 분리를 순서대로 적용하며 같은 API와 같은 데이터셋에서 단계별 병목 이동을 비교했습니다.
- **결과**: 차트 API의 문제를 하나의 느린 조회가 아니라 응답 경로 전체의 문제로 다시 정의했고, 최종적으로 조회 경로를 캐시·검색·폴백 경로·메타데이터 경로로 나눠 응답 병목을 줄였습니다.
...
차트는 단순 상위권 목록이 아니라, 사용자가 장르·연도·타입을 바꿔가며 음악을 탐색하는 공개 조회 경로였습니다. 한 릴리즈를 듣고 비슷한 장르 차트로 넘어가거나 특정 연도 상위권을 이어서 훑는 식의 연속 조회가 자연스럽게 발생했기 때문에, 이 API의 응답시간은 조회 1회 성능보다 탐색 흐름을 얼마나 끊지 않느냐에 더 가까운 문제였습니다.

또한 차트의 `lastUpdated`는 부가 정보가 아니라, 사용자가 이 결과를 현재 기준으로 신뢰해도 되는지를 설명하는 메타데이터였습니다. 응답이 느리거나 갱신 시각 조회가 불안정하면 탐색 경험뿐 아니라 공개 지표의 신뢰도도 함께 흔들릴 수 있었습니다.

구현 관점에서 차트 조회는 `chart_scores`를 기준으로 정렬했지만, 필터링과 응답 조립을 위해 릴리즈 기본 정보, 아티스트, 장르·디스크립터·언어 같은 분류 정보를 함께 읽거나 조인해야 했습니다.
...
결국 차트 API의 문제는, 조회 테이블 하나보다도 응답 경로 전체에 병목이 겹쳐 있었다는 점이었습니다. `chart_scores` 조회, 릴리즈·분류 정보 보강, 캐시 미스, 메타데이터 조회를 분리해 다뤄야 했습니다.

---

## 2. 설계 원칙

> 차트 서빙의 핵심은 `chart_scores`를 기준 조회 테이블로 두고, 그 위에 남아 있는 병목을 더 잘게 나누는 것입니다.

서빙 책임은 아래처럼 분리했습니다.

- `chart_scores`
  - 평점 집계 계층 `release_rating_summary`를 바탕으로 배치가 만들어 둔 기준 조회 테이블입니다.
- Redis 응답 캐시
...
- Redis와 Elasticsearch는 빠른 기본 조회 경로이고, MySQL 폴백 경로는 비싸지만 API를 끊기지 않게 만드는 안전망입니다.
- 갱신 시각 같은 공통 메타데이터도 실제 응답 시간에 영향을 주는 독립된 병목이므로, 검색 경로와 분리해 다뤄야 했습니다.

다중값 필터, 정렬, 페이지네이션이 겹친 조건은 MySQL 동적 조회만으로 안정적인 기본 조회 경로를 만들기 어려웠습니다. 그래서 Elasticsearch를 검색 경로로 두고, MySQL은 폴백 경로로 남겼습니다.
...
첫 단계는 정규화 조인으로 묶여 있던 조회 비용을 `chart_scores` 쪽으로 옮기는 일이었습니다. 기준 쿼리는 릴리즈 테이블을 중심으로, 조건에 따라 장르·디스크립터·언어 테이블까지 조인하는 구조였습니다. 정렬과 기본 필터에 필요한 점수, 릴리즈 타입, 발매 연도, 지역, 장르·디스크립터·언어 필터용 값은 `chart_scores`에 넣고, 최종 응답에 필요한 릴리즈 제목·발매일과 아티스트 이름은 복원 단계와 응답 조립 단계에 남겼습니다.
...
- 하지만 좁거나 복잡한 조건에서는 비정규화 폭이 커진 기준 조회 테이블만으로 충분하지 않았고, 이 한계는 부록의 전체 결과에서도 확인할 수 있습니다.
...
- 하지만 장르·디스크립터·언어 같은 JSON 다중값 필터는 인덱스 전략만으로 안정적인 기본 조회 경로가 되지 않았습니다.
...
이 지점에서 검색 경로를 Elasticsearch로 분리했습니다. MySQL 최적화를 적용한 뒤에도 JSON 다중값 필터가 포함된 미스 경로 병목이 남았기 때문입니다.

**이 단계의 결론:** 단일값 필터는 더 빨라졌지만, JSON 다중값 필터는 MySQL만으로 기본 조회 경로가 되기 어려웠습니다.

### 3-3. 반복 요청은 Redis가 흡수하고, 비싼 미스 경로는 별도로 확인했습니다

세 번째 단계는 Redis 응답 캐시입니다. 이 단계의 목적은 미스를 감추는 것이 아니라, 반복 요청의 기본 조회 경로와 비싼 미스 경로를 분리하는 것이었습니다.
...
네 번째 단계에서는 Elasticsearch를 검색 경로로 전환했습니다. Elasticsearch는 최종 응답을 만드는 대신, 필터 + 정렬 기준으로 릴리즈 ID 후보를 빠르게 찾는 역할만 맡았습니다.
...
- 차트는 원본 `ratings`를 다시 읽지 않고, 평점 집계 계층 `release_rating_summary`를 바탕으로 배치가 만든 `chart_scores`를 조회합니다.
- 빠른 기본 조회 경로는 Redis와 Elasticsearch이고, MySQL 폴백 경로는 검색 장애나 인덱스 미반영 상황에서도 API를 끊지 않기 위한 운영 안전장치입니다.
...
- 차트 최신성은 평점 집계 계층 `release_rating_summary`와 배치 반영 주기에 종속됩니다. 즉시 반영형 API가 아니라 배치 주기에 맞춰 갱신되는 구조입니다.
- 응답 조립 과정에서 릴리즈 조회와 아티스트 조회가 추가로 필요하므로, 검색 외 조회 비용은 계속 관리해야 합니다.

이 구조는 실시간 검색 플랫폼보다, 배치가 만든 차트 결과를 실제 API에서 빠르고 끊기지 않게 읽는 서빙 구조에 가깝습니다. 다음 단계에서는 이 구조를 분산 인스턴스와 고가용성 관점에서 얼마나 안정적으로 운영할 수 있는지까지 확장할 수 있습니다.

---

## 7. 관련 문서

- [평점 조회와 차트 배치가 함께 쓰는 요약 계층 만들기](./rating-aggregation.md)
...
**차트 API를 하나의 느린 조회로 다루지 않고, `chart_scores` 기반 조회 경로를 캐시·검색·폴백 경로·메타데이터 경로로 분리해 병목과 장애 영향을 경로별로 격리하는 서빙 구조로 재설계했습니다.**
