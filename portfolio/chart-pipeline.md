# 차트 생성·검증·공개를 분리해 공개 지표 신뢰도 지키기
...
#### Elasticsearch: 후보 인덱스와 공개 `alias`를 분리했습니다
...
종단 간 공개 검증으로 먼저 확인한 항목은 세 가지였습니다. 공개 단계가 끝난 시각과 API 노출 시각이 같은지, 공개가 끝난 뒤 Redis, Elasticsearch `alias`, API가 같은 공개 버전을 가리키는지, 롤백이나 Redis 키 유실 상황에서도 공개 기준점이 살아남는지였습니다. 결국 사용자가 보는 공개 지표가 끝까지 같은 기준을 따르는지 확인하는 절차였습니다.
...
- 공개가 안정화된 뒤에는 Redis 공개 버전, Redis 갱신 시각(`lastUpdated`), Elasticsearch `alias`, API 응답이 모두 같은 공개 버전을 가리켰습니다. 공개 지표 신뢰도가 저장소별로 갈라지지 않는지 확인할 수 있었습니다.
